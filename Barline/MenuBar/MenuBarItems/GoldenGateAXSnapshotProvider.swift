...
actor GoldenGateAXSnapshotProvider {
    ...
    private func snapshot(forceRefresh: Bool) async throws -> MenuBarSnapshot {
        ...
        result = logicalLayoutPlanner.applyingExplicitShelfOrder(
            to: result,
            assignments: effectiveAssignments
        )
        ...
        let items = snapshot.items.map { item in
            ...
            return item.replacing(
                isMovable: !item.isBarlineControlItem &&
                    (canReorderShelfItem || canAssignVisibility),
    ...
    private func logIdentityResolutionAudit(
        ...
        logger.notice(
            "Golden Gate identity table audit: source_entry_found=true, table_read_status=readable, publisher_team_resolved=\(publisherTeam != nil, privacy: .public), ax_team_resolved=\(axTeam != nil, privacy: .public), publisher_key_resolved=\(publisherKeyResolved, privacy: .public), ax_key_resolved=\(axKeyResolved, privacy: .public), globally_unique_source_key=\(resolvedKeys[source.id] != nil, privacy: .public), status_record_count=\(evidence.statusRecordCount, privacy: .public), bundle_record_count=\(evidence.bundleRecordCount, privacy: .public), bundle_record_suffix_parse_status=\(evidence.directBundleSuffixParseStatus.rawValue, privacy: .public), recognized_owner_record_count=\(evidence.recognizedOwnerRecordCount, privacy: .public), current_identity_origin=\(candidates.currentIdentityOrigin.rawValue, privacy: .public)"
        )
        logger.notice(
            "Golden Gate identity AX audit: ax_pid_matches_publisher=\(entry.axElementProcessIdentifier == entry.publisherProcessIdentifier, privacy: .public), publisher_still_running=\(publisherStillRunning, privacy: .public), source_element_still_valid=\(sourceElementStillValid, privacy: .public), publisher_live_entry_count=\(publisherLiveEntryCount, privacy: .public), children_read_status=\(childrenReadStatus.rawValue, privacy: .public), root_identifier_read_status=\(identityReadAudit.rootStatus(for: .identifier), privacy: .public), root_description_read_status=\(identityReadAudit.rootStatus(for: .accessibilityDescription), privacy: .public), root_title_read_status=\(identityReadAudit.rootStatus(for: .title), privacy: .public), child_identifier_read_counts=\(identityReadAudit.childCounts(for: .identifier), privacy: .public), child_description_read_counts=\(identityReadAudit.childCounts(for: .accessibilityDescription), privacy: .public), child_title_read_counts=\(identityReadAudit.childCounts(for: .title), privacy: .public)"
        )
        logger.notice(
            "Golden Gate descendant identity audit: descendant_node_count=\(descendantIdentityReadAudit.nodeCount, privacy: .public), descendant_children_read_counts=\(descendantIdentityReadAudit.childReadCounts(), privacy: .public), descendant_identifier_read_counts=\(descendantIdentityReadAudit.valueReadCounts(for: .identifier), privacy: .public), descendant_description_read_counts=\(descendantIdentityReadAudit.valueReadCounts(for: .accessibilityDescription), privacy: .public), descendant_title_read_counts=\(descendantIdentityReadAudit.valueReadCounts(for: .title), privacy: .public), descendant_identifier_value_count=\(descendantIdentityReadAudit.candidateCount(for: .accessibilityIdentifier), privacy: .public), descendant_identifier_accepted_key_count=\(descendantIdentifierEvidence.distinctAcceptedKeyCount, privacy: .public), descendant_description_value_count=\(descendantIdentityReadAudit.candidateCount(for: .accessibilityDescription), privacy: .public), descendant_description_accepted_key_count=\(descendantDescriptionEvidence.distinctAcceptedKeyCount, privacy: .public), descendant_title_value_count=\(descendantIdentityReadAudit.candidateCount(for: .accessibilityTitle), privacy: .public), descendant_title_accepted_key_count=\(descendantTitleEvidence.distinctAcceptedKeyCount, privacy: .public)"
        )
    }
