...
| --- | --- | --- |
| Pure domain | Swift Testing for snapshots, state coordination, profile presentation, display reconnect resolution, persistence/import, search, Spotlight records, and command/service validation | 430 tests pass for the exact 1.0.15 source candidate |
...
| UI smoke | Exact-build status-item, shelf Accessibility, and installed physical-equivalent journey probes | Four source-bound target journeys pass on macOS 26 and macOS 27; three XCUITests pass on macOS 26, while Xcode 27 skips two undeliverable synthetic status-item injections and the stronger physical journeys pass |
...
| Release/install/update | Clean archive, signing, notarization, stapling, Gatekeeper, Sparkle, SBOM, public download, and signed-upgrade gates | Build 76 passed exact-source packaging and public hash verification; signed 1.0.14 upgrades and clean DMG installs passed on both systems, and `v1.0.15` is public |
...
The published 1.0.15 (build 76) has exact signed-upgrade, native left/right,
...
The build-76 receipt set passed on a separate macOS 27 RC 26A428 host as well as
